...
import { navigate, parsePath, routePath } from './nav.js'
...
const parseRoute = () => parsePath(ALL_VIEWS)

function initialTheme() {
  const saved = localStorage.getItem('renv-theme')
...
export default function App() {
  ...
  const { slug, view, focus } = route            // slug now lives in the URL
  const setView = (v) => navigate(routePath(slug, v))
  const setSlug = (s) => navigate(routePath(s, view))   // switch project, keep the view
  const [theme, setTheme] = useState(initialTheme)
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem('renv-sidebar') === 'collapsed')
  const [sideW, setSideW] = useState(() => Number(localStorage.getItem('renv-sidebar-w')) || 228)
  ...
  const createProj = async () => {
    ...
    setNewProj({})
    await loadOverview()
    navigate(routePath(r.slug, 'overview'))
  }

  useEffect(() => {
    localStorage.setItem('renv-sidebar', collapsed ? 'collapsed' : 'open')
  }, [collapsed])
  ...
  useEffect(() => {   // the tab title tracks the view
    document.title = view.charAt(0).toUpperCase() + view.slice(1)
  }, [view])

  useEffect(() => {   // no project (or an unknown one) in the URL → first project
    if (!overview?.projects?.length) return
    if (!overview.projects.some((p) => p.slug === slug)) {
      history.replaceState(null, '', routePath(overview.projects[0].slug, view, focus))
      setRoute(parseRoute())
    }
  }, [overview, slug])

  useEffect(() => {   // Back/Forward + programmatic navigate() land here
  ...
  const loadOverview = useCallback(async () => {
    ...
    setOverview(o)   // the active project comes from the URL (normalized below)
  }, [])
  ...
  const openHit = (h) => {   // a result click deep-links into its view, in the
    const proj = h.project || slug   // hit's own project (may differ from active)
    const sub = {
    ...
    if (proj && sub) navigate('/' + encodeURIComponent(proj) + '/' + sub())
    setHits(null)
  ...
  const startResize = (e) => {
    ...
    const up = () => {
      ...
      localStorage.setItem('renv-sidebar-w', String(w))
    }
    ...
    <div className="app">
      ...
      <div className="main">
        <div className="topbar">
          ...
          <h2>
            <span className="crumb">{project ? project.slug : 'renv'} /</span>{' '}
