...
PROTECTED_NAMES = frozenset({"paper.tex", "preamble.tex"}) | GENERATED_NAMES
...
def _writable(name: str, kind: str = "file") -> bool:
    ...
    if suf == ".pdf" or suf in BUILD_SUFFIXES:
        return False
    return name not in GENERATED_NAMES and suf in WRITE_SUFFIXES
...
def read_file(con: sqlite3.Connection, corpus_root, slug: str, rel: str) -> dict:
    ...
    return {"path": rel, "content": content, "bytes": len(data),
            "generated": _is_generated(path.name), "writable": _writable(path.name)}
...
def scan_tree(con: sqlite3.Connection, corpus_root, slug: str) -> dict:
    """Parse every ``.tex`` file under ``text/`` for citation macros."""
    project_id(con, slug)
    text = _text_root(corpus_root, slug)
    spancites, cites, inputs = [], [], []
    for p in sorted(text.rglob("*.tex")):
        rel = p.relative_to(text).as_posix()
        parsed = parse_tex_macros(p.read_text(encoding="utf-8", errors="replace"),
                                  path=rel)
        spancites.extend(parsed["spancites"])
        cites.extend(parsed["cites"])
