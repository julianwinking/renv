...
contract is language-agnostic: the child reads ``REREF_RUN_DIR`` and
``REREF_PARAMS`` (JSON) from the environment and writes ``metrics.json`` (a flat
``{name: value}`` map) plus any artifact files into ``REREF_RUN_DIR``. We ingest
...
def _child_env(run_dir, params, seed, env_allow):
    ...
    env.update({"REREF_RUN_DIR": str(run_dir), "REREF_PARAMS": json.dumps(params),
                "REREF_SEED": str(seed)})
    return env
...
def _corpus_lock_hash(root) -> str | None:
    ...
    lock = Path(root) / ".reref" / CONFIG_FILENAME
    if not lock.exists():
        return None
    try:
...
def start_run(con, project, slug, *, entrypoint, root=".", params=None,
    ...
    def worker():
        wcon = __import__("reref.db", fromlist=["connect"]).connect(root)
        try:
