# reref cockpit (React Flow graph UI)

An interactive graph view over the `reref` research store — experiment branches
(DAG), claims, findings, citations, and papers as connected, expandable nodes.
Talks to the Python backend's JSON API (`reref web`). The store stays the single
source of truth; this is just a richer frontend.

## Develop
Two processes:
```bash
reref web                    # backend JSON API at http://127.0.0.1:8765
...
## Build (served by `reref web`)
```bash
cd cockpit && npm install && npm run build   # → cockpit/dist
reref web                                     # serves the built app at http://127.0.0.1:8765
```
`reref web` serves `cockpit/dist/` if present, else falls back to the lightweight
buildless page in `reref/web/`.
