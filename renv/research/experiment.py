...
from renv.config import CONFIG_FILENAME, sha256_file
from renv.research.db import get_or_create_config, now, project_id, row_to_dict
...
def start_run(con, project, slug, *, entrypoint, root=".", params=None,
    ...
    def worker():
        from renv.research import db as _db
        wcon = _db.connect(root)
        try:
...
def ingest_run(con: sqlite3.Connection, project: str, slug: str, *,
    ...
    if remote:
        from renv.research.remote import expand_locator
