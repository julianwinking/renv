...
import { paperPdfUrl, getPaperAnchors, addPaperNote, updatePaperNote, deletePaperNote } from '../api.js'
import { findRange, findAllRanges, paintRects, selectionAnchor } from '../pdfhl.js'
...
const KINDS = [
  ['note', 'Note', 'Add a note at this position…'],
...
export default function PaperViewer({ paperKey, title, project, onClose, onMutate, embedded = false,
  ...
  const [panelW, setPanelW] = useState(() => Number(localStorage.getItem('reref-pv-panel')) || 264)
  const [showSide, setShowSide] = useState(() => localStorage.getItem('reref-pv-side') !== 'off')
  const [sideW, setSideW] = useState(() => Number(localStorage.getItem('reref-pv-sidew')) || 300)
  ...
  // re-render pages when the zoom changes, keeping you on the same page: capture
  // the current page + how far you're scrolled into it, then restore that after
  // the re-render (the page's pixel height changes, the fraction doesn't).
  useEffect(() => {
    ...
    if (cont && pgs.length) {
      ...
      const pg = [...pgs].reverse().find((p) => p.pageEl.offsetTop <= y + 1) || pgs[0]
      anchor = { page: pg.page, frac: (y - pg.pageEl.offsetTop) / (pg.pageEl.offsetHeight || 1) }
    }
    renderPages(scale).then(() => {
      repaintHighlights()
      if (anchor && cont) {
        const pg = pagesRef.current[anchor.page - 1]
        if (pg) cont.scrollTop = pg.pageEl.offsetTop + anchor.frac * pg.pageEl.offsetHeight
      }
    })
  }, [scale])
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      const dir = acc > 0 ? -1 : 1
      acc = 0
      setScale((s) => Math.min(3, Math.max(0.6, Math.round((s + dir * 0.12) * 100) / 100)))
    }
  ...
  const startPanelResize = (e) => {
    ...
    const up = () => {
      ...
      localStorage.setItem('reref-pv-panel', String(w))
    }
  ...
  const startSideResize = (e) => {           // right sidebar: drag its left edge
    ...
    const up = () => {
      window.removeEventListener('mousemove', move)
      window.removeEventListener('mouseup', up)
      localStorage.setItem('reref-pv-sidew', String(w))
    }
    window.addEventListener('mousemove', move)
    window.addEventListener('mouseup', up)
  }
  ...
  useEffect(() => { if (status === 'ready') repaintHighlights() }, [anchors, status])
  ...
  async function renderPages(useScale) {
    ...
    for (let i = 1; i <= doc.numPages; i++) {
      ...
      await tl.render()
      pagesRef.current.push({ page: i, pageEl, textLayerEl, hlEl })
    }
  }
  ...
  function repaintHighlights() {
    ...
    for (const pg of pagesRef.current) {
      pg.hlEl.querySelectorAll('.pv-mark-cite, .pv-mark-note').forEach((el) => el.remove())
  ...
  const onDocClick = (e) => {
    const s = window.getSelection()
    if (s && !s.isCollapsed) return
    const mark = document.elementsFromPoint(e.clientX, e.clientY)
      .find((el) => el.classList?.contains('pv-mark') && !el.classList.contains('pv-mark-pending'))
    if (!mark) return
    if (mark.dataset.kind === 'note') { setOpenNote(Number(mark.dataset.id)); focusSidebar(`note-${mark.dataset.id}`) }
    else focusSidebar(`cite-${mark.dataset.id}`)
  }
  ...
  const inner = (
    <>
      <div className="pv-bar">
        ...
        <div className="pv-centertools">
          <span className="pv-zoom">
            <button className="pv-tbtn" title="Zoom out" onClick={() => zoom(-0.15)}>−</button>
            <span className="pv-zoom-n">{Math.round(scale * 100)}%</span>
            <button className="pv-tbtn" title="Zoom in" onClick={() => zoom(0.15)}>＋</button>
          </span>
        ...
        <div className="pv-bar-meta">
          ...
          <button className={`pv-tbtn ${showSide ? 'on' : ''}`} title="Toggle annotations panel"
                  onClick={() => { const v = !showSide; setShowSide(v); localStorage.setItem('reref-pv-side', v ? 'on' : 'off') }}>
            <PanelIco side="right" />
          </button>
      ...
      <div className="pv-body">
        ...
        <div className="pv-doc" ref={docScrollRef} onScroll={onDocScroll} onMouseUp={onMouseUp} onClick={onDocClick}>
              ...
              {openNote === n.id && (
                <div className="pv-note-body">
                  ...
                  <div className="pv-note-tools">
                    ...
                    <button className="btn ghost danger" onClick={() => removeNote(n)}>Delete</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {popover && (
        <div className="pv-pop" style={{ left: popover.clientX, top: popover.clientY + 8 }}
          ...
          <div className="pv-pop-tools">
            ...
            <button className="btn" onClick={saveNote}>Add {draft.kind}</button>
          </div>
          {onCite && citeTargetTitle && (
