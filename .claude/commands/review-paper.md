...
Run `uv run reref review $1`. These are deterministic facts cross-checked against
...
Fetch the rubric: call the `rubric` MCP tool (or read `reref/review.py:RUBRIC`).
...
3 highest-leverage fixes. If `reref review` had HIGH findings, state that the
