...
- **Provenance, enforced.** A `result` log entry is rejected unless it links a
  completed run, and `reref log check` re-audits the whole store. This
...
- **Numbers are woven, never typed.** `reref weave` regenerates the results
...
uv run reref cite "..." projects/span-citation --write   # -> citations.json
...
index the demo library with 'uv run reref index', then show me my first
span-anchored citation with 'uv run reref cite'.
```

## How it works

One shared corpus, many papers. The engine (`reref/`) and the corpus
(`library/` plus the derived `.reref/` index) live at the repo root. Each
...
`.reref/reref.lock.json` pins parser, chunker, and embedder versions plus a
...
uv run reref exp run span-citation 001-tfidf --entrypoint run.py --param k=8
...
uv run reref log check                        # audit the provenance invariant
...
uv run reref claim link 1 --cite 3 --stance supports
uv run reref claim list span-citation         # status is DERIVED from evidence

uv run reref weave span-citation              # regenerate results table + bibliography
...
```bash
cd cockpit && npm install && npm run build   # build once; reref web serves it
```
...
`.mcp.json` registers a local stdio MCP server (`reref mcp`, pure stdlib) so
...
for the workflow and the design invariants PRs are reviewed against, and
[`SECURITY.md`](SECURITY.md) for reporting vulnerabilities. CI runs the test
suite on the oldest and newest supported Python plus a cockpit build.

## Citation

If renv is useful in your research, please cite it (see
[`CITATION.cff`](CITATION.cff)). Published projects on renv.ai additionally
get a per-project citation with a stable URL.
