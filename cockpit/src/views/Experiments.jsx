  ...
  return (
    <>
      ...
      <Section title="Branches" aside={`${exps.length} experiments · ${runs.length} runs`}>
            ...
            <div key={e.id} id={`exp-${e.slug}`} className={focus === e.slug ? 'flash' : ''}>
              ...
              {isOpen && (
                <div className="detail">
                  ...
                  {!eruns.length && <div className="muted">No runs yet — <span className="mono">renv exp run {slug} {e.slug} …</span></div>}
                </div>
              )}
            </div>
          )
        ...
        {!exps.length && (
          <Empty>No experiments yet — <code>renv exp new {slug} 001-… --hypothesis "…"</code> opens the first branch.</Empty>
        )}
      </Section>
