...
Tag format (in any comment):
    ...
    @reref:<kind>:<id>[:<relation>] [free text]
...
Examples:
    # @reref:finding:42:fixes  guard malformed metrics.json (review HIGH)
    # @reref:paper:gao2023_alce:implements  ALCE citation-precision metric
    # @reref:decision:17  per the sentence-anchor decision
"""
...
TAG_RE = re.compile(
    r"@reref:(?P<kind>paper|finding|decision|run|claim|dataset|experiment)"
    r":(?P<id>[\w./-]+)(?::(?P<relation>[\w-]+))?(?:[ \t]+(?P<text>[^\n]*))?"
)
_SKIP_DIRS = {".git", ".venv", "venv", "__pycache__", ".reref", ".research",
...
def scan(root=".") -> list[dict]:
    """Find every @reref tag in the tree. Returns file/line-located references."""
    root = Path(root)
    out = []
...
def strip_text(text: str) -> str:
    """Remove @reref tags; drop comment lines that become empty as a result."""
    out = []
