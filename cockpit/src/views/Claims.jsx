  ...
  return (
    <>
      ...
      <Section title="Claim ledger" aside={`${claims.length} claims`}>
            ...
            {detail[c.id] && (
              <div className="detail">
                ...
                {!(detail[c.id].evidence || []).length && (
                  <div className="muted">No evidence linked — <span className="mono">renv claim link {c.id} --run N</span></div>
                )}
              </div>
            )}
          </div>
        ...
        {!claims.length && (
          <Empty>No claims yet — <code>renv claim add {slug} "…" --kind thesis</code> states what the runs must prove.</Empty>
        )}
      </Section>
