...
import { getPapers, getPaperAnchors, addPaper, getPaperDocs, createPaperDoc } from '../api.js'
...
export default function Papers({ focus, slug, onMutate }) {
  ...
  const [tabs, setTabs] = useState([])              // [{ type:'paper'|'doc', key, title, docId? }]
  const [active, setActive] = useState('library')
  const [split, setSplit] = useState(null)          // tab key shown in the right pane
  const [ratio, setRatio] = useState(() => Number(localStorage.getItem('reref-pp-ratio')) || 55)
  ...
  const startSplitResize = (e) => {
    ...
    const up = () => {
      ...
      localStorage.setItem('reref-pp-ratio', String(r))
    }
    ...
    <div className="lib-wrap">
      ...
      {!papers.length ? (
        <div style={{ padding: 24 }}>
          <Empty>The corpus is empty — <code>reref add &lt;pdf|arxiv-id|doi&gt;</code> or the <b>+ Add</b> button brings papers in.</Empty>
        </div>
      ) : (
                            ...
                            <>
                              {ex.notes.map((n) => (
                                <div key={`n${n.id}`} className="lib-anno" onClick={() => onOpen(p)}>
                                  <span className={`pv-kindtag pv-kt-${n.kind || 'note'}`}>{n.kind || 'note'}</span>
