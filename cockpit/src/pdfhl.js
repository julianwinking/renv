...
// parsed reference numbers) filters out equation/section refs like [12] in math.
const REF_MARKER_RE = /\[(\d{1,3}(?:\s*[,–—-]\s*\d{1,3})*)\]/g
export function findRefMarkers(textLayerEl, validNums) {
  const { text, nodeAt } = buildIndex(textLayerEl)
  const out = []
  REF_MARKER_RE.lastIndex = 0
  let m
  while ((m = REF_MARKER_RE.exec(text)) && out.length < 400) {
    const nums = []
    for (let part of m[1].replace(/[–—]/g, '-').split(',')) {
      part = part.trim()
      const r = part.match(/^(\d+)\s*-\s*(\d+)$/)
      if (r) {
        for (let n = +r[1]; n <= +r[2] && nums.length < 40; n++) nums.push(n)
      } else if (/^\d+$/.test(part)) nums.push(+part)
    }
    const known = nums.filter((n) => validNums.has(n))
