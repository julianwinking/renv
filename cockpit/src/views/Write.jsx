...
function TreeNode({ node, depth, open, toggle, active, onOpen }) {
  if (node.kind === 'dir') {
    ...
    return (
      <div>
        <button className="tx-fn tx-dir" style={{ paddingLeft: 8 + depth * 12 }}
                onClick={() => toggle(node.path)}>
          <span className="tx-chev">{shown ? '▾' : '▸'}</span>
          {node.name}
        </button>
        {shown && (node.children || []).map((c) => (
  ...
  return (
    ...
    >
      ...
      {node.build && <span className="tx-tag">pdf</span>}
    </button>
  )
}
...
export default function Write({ slug, focus }) {
  ...
  return (
    <div className="tx">
      <aside className="tx-files" style={{ width: treeW }}>
        <div className="tx-files-h">
          <span className="eyebrow" style={{ margin: 0 }}>text/</span>
        ...
        {newName !== null && (
          <div className="tx-new">
            ...
            <button className="btn" onClick={createFile}>Add</button>
          </div>
        )}
        {tree ? <FileTree tree={tree.tree} active={path} onOpen={(n) => { if (n.kind === 'file' && !n.build) openPath(n.path) }} />
      ...
      <div className="tx-mid">
        <div className="tx-bar">
          <span className="mono tx-path">{path}</span>
          {file?.generated && <span className="tx-tag">generated — weave</span>}
          {dirty && <span className="faint">unsaved</span>}
          {saving && <span className="faint">saving…</span>}
          <div className="tx-bar-actions">
            <button className="btn ghost" onClick={weaveOnly} title="Regenerate results_table.tex and references.bib from the store">Weave</button>
            <button className="btn" onClick={recompile} disabled={compile.status === 'running'}
                    title="⌘/Ctrl+Enter — weave, then compile">
              {compile.status === 'running' ? 'Compiling…' : 'Recompile'}
            </button>
            {file?.writable && path !== 'paper.tex' && path !== 'preamble.tex' && (
              <button className="btn ghost" onClick={removeFile}>Delete</button>
            )}
          </div>
        </div>
          ...
          {panel === 'cite' && (
            <div className="tx-ppane">
              <div className="muted" style={{ fontSize: 12, marginBottom: 6 }}>
                Ground a sentence in a quoted span from the corpus index — this writes a
                {' '}<span className="mono">\spancite</span> (not a bare <span className="mono">\cite</span>)
                and a citation row you can link to a claim.
              </div>
              ...
              <div className="tx-cite-row">
                ...
                <button className="btn" disabled={searching} onClick={() => runCite(false)}>
            ...
            <div className="tx-ppane">
              ...
              {picked && picked.source_id && (
                <div className="tx-picked">
                  ...
                  <button className="btn ghost" onClick={() => navigate(routePath(slug, 'papers', picked.source_id))}>
                    Open source paper
                  </button>
                </div>
              )}
          ...
          {panel === 'metrics' && (
            <div className="tx-ppane">
              <div className="muted" style={{ fontSize: 12, marginBottom: 8 }}>
                Numbers in the paper come from recorded runs via <span className="mono">renv weave</span>.
                {' '}<span className="mono">{'\\input{results_table}'}</span> is the only table — never type a metric.
              </div>
              <button className="btn ghost" onClick={() => ed.current?.insert('\\input{results_table}\n')}>
                Insert results table
              </button>
              <button className="btn ghost" onClick={weaveOnly}>Weave now</button>
              ...
              {(ctx?.claims || []).length > 0 && (
                <>
                  <div className="eyebrow" style={{ marginTop: 12 }}>claims</div>
                  {(ctx.claims).map((c) => (
