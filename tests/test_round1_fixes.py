...
from reref import db, experiment, ingest, log, refs
from reref.dataset import register_dataset
...
def test_malformed_metrics_marks_failed_not_running(tmp_path):
    con = _exp(tmp_path)
    bad = tmp_path / "bad.py"
    bad.write_text("import os\nopen(os.environ['REREF_RUN_DIR']+'/metrics.json','w').write('not json')\n")
    with pytest.raises(RuntimeError):
...
def test_nonscalar_metric_rejected(tmp_path):
    con = _exp(tmp_path)
    bad = tmp_path / "b.py"
    bad.write_text("import json,os\njson.dump({'x':[1,2]}, open(os.environ['REREF_RUN_DIR']+'/metrics.json','w'))\n")
    with pytest.raises(RuntimeError):
        experiment.run_experiment(con, "p", "001", entrypoint=str(bad), root=str(tmp_path))
...
def test_refs_scan_and_strip(tmp_path):
    ...
    f.write_text(
        "def fix():\n"
        "    # @reref:finding:42:fixes  guard malformed metrics.json\n"
        "    return 1  # @reref:paper:gao2023_alce  ALCE metric\n")
    ...
    assert "@reref" not in stripped
    assert "return 1" in stripped                    # code survives
    assert "    # @reref:finding" not in stripped     # pure-tag comment line dropped
