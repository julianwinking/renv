...
from reref.mcp_server import TOOLS_BY_NAME, handle
...
def test_initialize_echoes_protocol(tmp_path):
    ...
    assert resp["result"]["serverInfo"]["name"] == "reref"
...
def test_stdio_handshake_subprocess(tmp_path):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "reref.cli", "--corpus", str(tmp_path), "mcp"],
        input=msgs, capture_output=True, text=True, cwd=str(root), timeout=30)
    lines = [json.loads(l) for l in proc.stdout.splitlines() if l.strip()]
    assert lines[0]["result"]["serverInfo"]["name"] == "reref"   # initialize reply
