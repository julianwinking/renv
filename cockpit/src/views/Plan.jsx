...
import { getPlan, addPlanItem, updatePlanItem, deletePlanItem, getProject,
         getPhases, setPhaseBandColor } from '../api.js'
import { asArray, Stamp, Confirm } from '../ui.jsx'
import { REGION_COLORS } from '../nodes.jsx'
...
export default function Plan({ slug }) {
  ...
  const [labelW, setLabelW] = useState(() =>
    Number(localStorage.getItem('renv-gantt-labels')) || 230)
  ...
  const startLabelResize = (e) => {
    ...
    const up = () => {
      ...
      localStorage.setItem('renv-gantt-labels', String(w))
    }
  ...
  // phases that have a graph band carry the band's color here too — square,
  // bar ring, and the graph bands all speak the same color (phase_band.color)
  const [bands, setBands] = useState({})           // plan_item_id → {cls}
  const [colorMenu, setColorMenu] = useState(null) // plan_item_id with open picker
  const load = () => Promise.all([getPlan(slug), getPhases(slug)]).then(([i, ph]) => {
    setItems(asArray(i))
    const placed = asArray(ph).filter((p) => p.x0 != null)
    setBands(Object.fromEntries(placed.map((p, idx) =>
      [p.id, { cls: p.color ? 'phase-' + p.color : 'phase-c-' + (idx % 6) }])))
  })
  useEffect(() => {
      ...
      {items.length > 0 && (
          <div className="gantt gantt-page">
            <div className="gantt-labels" style={{ width: labelW }}>
              ...
              <div className="gantt-head"><span className="eyebrow" style={{ margin: 0 }}>Phases</span></div>
              {visible.map((it) => {
                const band = bands[it.id]
                return (
                <div key={it.id} className="gantt-labelwrap">
                <button className="gantt-label rowbtn"
                  ...
                  <span className={`gantt-swatch ${band ? band.cls : ''}`}
                        style={band
                          ? { background: 'var(--pb-text)', cursor: 'pointer' }
                          : { background: TONE_COLOR[itemState(it, today)] }}
                        title={band ? 'Phase color — click to change (synced with the graph band)' : undefined}
                        onClick={band ? (e) => {
                          e.stopPropagation()
                          setColorMenu(colorMenu === it.id ? null : it.id)
                        } : undefined} />
                ...
                {colorMenu === it.id && (
                  <div className="region-menu gantt-colormenu">
                    {REGION_COLORS.map((c) => (
                      <button key={c} className={`region-swatch region-${c}`} title={c}
                              onClick={() => { setColorMenu(null); setPhaseBandColor(it.id, c).then(load) }} />
                    ))}
                  </div>
                )}
                </div>
                      ...
                      <div key={it.id} className="gantt-row">
                        {it.kind === 'phase' && (
                          <div
                            ...
                            className={`gantt-bar bar-${state} ${band ? band.cls : ''}`}
                            style={{ left: x(disp.start || disp.due),
                                     width: Math.max(x(disp.due) - x(disp.start || disp.due) + PX, PX),
                                     ...(band ? { borderColor: 'var(--pb-text)' } : null) }}
