...
def serve(root=".", port: int = 8765, host: str = "127.0.0.1", *,
    ...
    if launchd:
        ...
        print(f"reref cockpit (launchd-activated, {len(fds)} socket(s), "
        ...
        print(f"reref cockpit → {scheme}://{host}:{port}   (Ctrl-C to stop)")
...
_HOSTS_BEGIN = "# >>> reref cockpit (managed — reref web install) >>>"
_HOSTS_END = "# <<< reref cockpit (managed) <<<"
...
def update_hosts(domains: list[str], *, apply: bool = True) -> bool:
    ...
    import subprocess
    import tempfile
    if not (_HOSTS.parent / "hosts.reref.bak").exists():
        subprocess.run(["sudo", "cp", str(_HOSTS), "/etc/hosts.reref.bak"], check=True)
