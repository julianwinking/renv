...
const DAY = 86400000
// same palette as the Gantt activity lane
const ACT_COLOR = {
...
const fmtRange = (d) => new Date(d + 'T00:00:00Z')
const fmtDay = (d) => fmtRange(d)
  .toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' })
...
export default function Overview({ slug, project, defs, counts }) {
  ...
  // daily activity, stacked by log-entry type, from the project start to today
  const activity = {}
  for (const e of [...(data.log || []), ...(data.notes || []).map((n) => ({ ...n, type: 'note' }))]) {
    const day = (e.ts || '').slice(0, 10)
    if (!day) continue
    activity[day] = activity[day] || {}
    activity[day][e.type] = (activity[day][e.type] || 0) + 1
  }
  const actDays = Object.keys(activity)
  const startDay = (project?.created || '').slice(0, 10) || (actDays.length ? actDays.slice().sort()[0] : today)
  const span = Math.max(1, Math.round((fmtRange(today) - fmtRange(startDay)) / DAY))
  const maxTotal = Math.max(1, ...Object.values(activity).map((c) => Object.values(c).reduce((a, b) => a + b, 0)))
  ...
  return (
    <>
      ...
      {actDays.length > 0 && (
        <div className="act" title="Logged activity per day, from the project start to today">
          <div className="act-track">
            {Object.entries(activity).map(([day, counts]) => {
              const total = Object.values(counts).reduce((a, b) => a + b, 0)
              const x = Math.min(99, Math.max(0, (fmtRange(day) - fmtRange(startDay)) / DAY / span * 100))
              const tip = `${fmtDay(day)} — ` + Object.entries(counts).map(([t, n]) => `${n} ${t}`).join(', ')
              let acc = 0
              return (
                <span key={day} className="act-col" style={{ left: `${x}%` }} title={tip}>
                  {Object.entries(counts).map(([t, n]) => {
                    const h = (n / maxTotal) * 100
                    const seg = <span key={t} className="act-seg" style={{ bottom: `${acc}%`, height: `${h}%`, background: ACT_COLOR[t] || 'var(--citation)' }} />
                    acc += h
                    return seg
                  })}
                </span>
      ...
      <div className="grid cols-2">
        <Section title="Latest runs" aside={`${runs.length} recorded`}>
          ...
          {!runs.length && (
            <Empty>No runs recorded yet — <code>renv exp run …</code> puts the first number in the ledger.</Empty>
          )}
        </Section>
        ...
        <Section title="Claims" aside="Status derived from evidence">
          ...
          {!claims.length && (
            <Empty>No claims yet — <code>renv claim add</code> states the thesis your runs must back.</Empty>
          )}
        </Section>
      </div>
      ...
      <Section title="Recent log" aside="Newest first">
        ...
        {!log.length && (
          <Empty>The decision log is empty — <code>renv log add …</code> records the why before the work.</Empty>
        )}
      </Section>
    </>
  )
